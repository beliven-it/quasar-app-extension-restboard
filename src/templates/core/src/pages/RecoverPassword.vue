<template>
  <q-card class="q-pa-sm shadow-1">
    ...
    <q-card-actions class="row justify-between">
      <q-btn flat dense no-caps :label="$t('Back to sign in')" @click="onCancel" />
      <q-btn color="primary" :label="$t('Recover')" @click="onRecoverPasswordRequest" />
    </q-card-actions>
  </q-card>
</template>
...
export default defineComponent({
  ...
  methods: {
    ...
    onRecoverPasswordRequest() {
      this.$store.dispatch('core/recoverPassword', this.username)
